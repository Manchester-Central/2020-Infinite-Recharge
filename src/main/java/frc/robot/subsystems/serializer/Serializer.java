...
public class Serializer extends SubsystemBase implements ISerializer {
  ...
  public double maxRPM = 1500;
  private double setPoint = 0;
  private final double FAST_SPEED = 2200;
  private final double SLOW_SPEED = 600;
  private double manualSpeedTarget = 900;
  private double rotationLength = 11.202576;
  ...
  public Serializer() {
    ...
    /*
     * m_pidController.setIZone(kIz); m_pidController.setFF(kFF);
     * m_pidController.setOutputRange(kMinOutput, kMaxOutput);
     */

    if (tuning) {
      putTuningValuesOnDashboard();
    }

    SmartDashboard.putNumber("Serializer Target", manualSpeedTarget);
  }

  private void putTuningValuesOnDashboard() {
  ...
  private void updateTuningValuesFromDashboard() {
  ...
  // double gearRatio = (double) 10 / 1; // ratio of the axel the turntable lies
  // on to the axel the encoder reads
  // int ticksPerRev = 42; // amount of ticks in one revolution of the encoder
  // axel
  ...
  public void driveTurnTable(SerializerSpeed speed) {
    ...
    updateTuningValuesFromDashboard();
    }
